...
function naturalSupplementalExample(level: CEFRLevel, term: TermSeed, index: number) {
  const word = term.pt;
  const lowerWord = word.toLocaleLowerCase('pt-BR');
  const seed = [...lowerWord].reduce((sum, character) => sum + character.charCodeAt(0), index);
  const isPlural = term.zh.includes('复数');
  const specificExamples: Partial<Record<CEFRLevel, Record<string, string>>> = {
    A1: {
      nome: 'Meu nome aparece no formulário.',
      número: 'Anotei o número no celular.',
      endereço: 'O endereço fica perto da praça.',
      horário: 'O horário da aula mudou.',
      preço: 'O preço do café está na placa.',
      pedido: 'O pedido chegou à mesa.',
      café: 'Bebo café de manhã.',
      água: 'Levo água na mochila.',
      suco: 'Quero um suco de laranja.',
      lanche: 'Comprei um lanche antes da aula.',
      mesa: 'A mesa fica perto da janela.',
      chave: 'A chave está na bolsa.',
      porta: 'A porta da sala está aberta.',
      janela: 'Abro a janela de manhã.',
      rua: 'Minha rua é tranquila.',
      praça: 'A praça fica no centro.',
      loja: 'A loja abre cedo.',
      mercado: 'Vou ao mercado depois da aula.',
      ônibus: 'O ônibus passa às oito.',
      metrô: 'Uso o metrô para ir ao trabalho.',
      bilhete: 'Comprei o bilhete na estação.',
      mala: 'Minha mala está pesada.',
      quarto: 'O quarto tem uma cama grande.',
      banheiro: 'O banheiro fica à direita.',
      telefone: 'Meu telefone está sem bateria.',
      mensagem: 'Recebi uma mensagem da professora.',
      amigo: 'Meu amigo estuda português comigo.',
      família: 'Minha família mora longe.',
      aula: 'A aula começa às nove.',
      livro: 'Leio um livro em português.',
    },
    A2: {
      vizinhança: 'A vizinhança é tranquila durante a semana.',
      confirmações: 'As confirmações chegaram por mensagem.',
      cancelamentos: 'Os cancelamentos aparecem no aplicativo.',
      reservas: 'As reservas do hotel ficam no meu e-mail.',
      pagamentos: 'Os pagamentos podem ser feitos com cartão.',
      entregas: 'As entregas chegam pela manhã.',
      trocas: 'As trocas são feitas no balcão da loja.',
      devoluções: 'As devoluções precisam da nota fiscal.',
      agendamento: 'O agendamento da consulta ficou para terça-feira.',
      atraso: 'O atraso do ônibus mudou meus planos.',
      mudança: 'A mudança de horário apareceu no aviso.',
      preferência: 'Minha preferência é pagar com cartão.',
      localização: 'A localização do hotel aparece no mapa.',
      confirmação: 'A confirmação da reserva chegou por e-mail.',
      cancelamento: 'O cancelamento do voo saiu no aplicativo.',
      reserva: 'A reserva do hotel está no meu nome.',
      pagamento: 'O pagamento foi aprovado no cartão.',
      entrega: 'A entrega chegou antes do almoço.',
      troca: 'A troca do produto foi rápida.',
      devolução: 'A devolução precisa da nota fiscal.',
      descrição: 'A descrição do apartamento é bem clara.',
      comparação: 'A comparação ajudou na escolha.',
      organização: 'A organização da viagem levou uma semana.',
      preparação: 'A preparação para a entrevista começou cedo.',
      escolha: 'A escolha do horário depende da agenda.',
      sugestão: 'A sugestão do professor foi útil.',
      convite: 'O convite da festa chegou hoje.',
      mensagem: 'A mensagem explica o novo horário.',
      documento: 'O documento fica salvo no celular.',
      formulário: 'O formulário pede nome e endereço.',
    },
  };

  const exact = specificExamples[level]?.[lowerWord];
  if (exact) return exact;

  const templates: Record<CEFRLevel, Array<(term: string) => string>> = {
    A1: [
      term => `Uso ${term} em situações simples do dia a dia.`,
      term => `Procuro ${term} quando preciso resolver algo rápido.`,
      term => `Tenho ${term} perto de casa.`,
      term => `Levo ${term} comigo quando saio.`,
      term => `Vejo ${term} no caminho para a aula.`,
      term => `Comprei ${term} no mercado do bairro.`,
      term => `Preciso de ${term} antes de sair.`,
      term => `Falo sobre ${term} com um amigo.`,
      term => `Guardo ${term} na bolsa.`,
      term => `Encontro ${term} perto da estação.`,
      term => `Escrevo ${term} no caderno.`,
      term => `Peço ${term} quando estou no café.`,
    ],
    A2: [
      term => `Anotei ${term} na minha agenda.`,
      term => `Falei sobre ${term} com a atendente.`,
      term => `Resolvi a situação de ${term} pelo aplicativo.`,
      term => `Guardei ${term} no e-mail para consultar depois.`,
      term => `Perguntei sobre ${term} antes de confirmar o plano.`,
      term => `A informação sobre ${term} ficou clara na mensagem.`,
      term => `Organizei ${term} antes da viagem.`,
      term => `Expliquei ${term} com calma para a funcionária.`,
      term => `Recebi um aviso sobre ${term} pela manhã.`,
      term => `Comparei ${term} antes de escolher.`,
      term => `Usei ${term} para resolver o problema.`,
      term => `A pessoa confirmou ${term} por telefone.`,
      term => `Preciso verificar ${term} antes de pagar.`,
      term => `Coloquei ${term} na lista de tarefas.`,
    ],
    B1: [
      term => `No projeto, discutimos ${term} com exemplos concretos.`,
      term => `O tema de ${term} apareceu na reunião da equipe.`,
      term => `Usei ${term} para explicar melhor a minha opinião.`,
      term => `A conversa sobre ${term} ajudou na decisão final.`,
      term => `A experiência com ${term} mudou meu ponto de vista.`,
      term => `Escrevi um resumo sobre ${term} para a aula.`,
      term => `O grupo comparou ${term} com outras soluções.`,
      term => `A professora pediu uma opinião sobre ${term}.`,
      term => `A situação mostrou a importância de ${term}.`,
      term => `Aprendi a relacionar ${term} com problemas reais.`,
      term => `O debate sobre ${term} foi mais difícil do que esperávamos.`,
      term => `A notícia trouxe um exemplo claro de ${term}.`,
      term => `Meu argumento principal depende de ${term}.`,
      term => `A decisão final levou ${term} em consideração.`,
    ],
    B2: [
      term => `A equipe avaliou ${term} antes de definir a estratégia.`,
      term => `O relatório relaciona ${term} com os resultados do trimestre.`,
      term => `A proposta exige atenção a ${term} e aos seus impactos.`,
      term => `Durante a análise, ${term} foi tratado como prioridade.`,
      term => `A pesquisa mostra como ${term} afeta diferentes grupos.`,
      term => `O plano inclui indicadores para acompanhar ${term}.`,
      term => `A gestão de ${term} exige critérios transparentes.`,
      term => `O debate público sobre ${term} ganhou força este ano.`,
      term => `A empresa revisou ${term} depois da auditoria.`,
      term => `Os dados ajudam a medir ${term} com mais precisão.`,
      term => `A solução proposta reduz riscos ligados a ${term}.`,
      term => `A comparação entre setores esclarece ${term}.`,
      term => `A decisão depende da viabilidade de ${term}.`,
      term => `O estudo destaca limitações importantes de ${term}.`,
    ],
    C1: [
      term => `O artigo examina ${term} em diálogo com o contexto social.`,
      term => `A discussão sobre ${term} exige uma leitura mais cuidadosa.`,
      term => `A autora mobiliza ${term} para sustentar o argumento central.`,
      term => `O debate revela como ${term} altera a interpretação do problema.`,
      term => `A análise de ${term} depende de uma delimitação conceitual rigorosa.`,
      term => `O texto articula ${term} com transformações institucionais recentes.`,
      term => `A abordagem de ${term} evita explicações excessivamente simples.`,
      term => `A pesquisa situa ${term} dentro de uma tradição crítica.`,
      term => `A interpretação de ${term} muda conforme o recorte adotado.`,
      term => `O seminário problematizou ${term} a partir de casos concretos.`,
      term => `A leitura de ${term} revela tensões entre teoria e prática.`,
      term => `O argumento ganha força quando relaciona ${term} a evidências empíricas.`,
      term => `A autora distingue ${term} de conceitos próximos.`,
      term => `O estudo mostra os desdobramentos de ${term} no debate contemporâneo.`,
    ],
    C2: [
      term => `O ensaio problematiza ${term} sem reduzir sua complexidade.`,
      term => `A conferência abordou ${term} a partir de uma perspectiva crítica.`,
      term => `A análise articula ${term} com disputas teóricas contemporâneas.`,
      term => `O conceito de ${term} exige precisão terminológica e histórica.`,
      term => `A investigação de ${term} revela camadas conceituais sobrepostas.`,
      term => `O autor reinscreve ${term} em uma genealogia mais ampla.`,
      term => `A tensão em torno de ${term} atravessa diferentes campos do saber.`,
      term => `A crítica de ${term} evita tanto simplificações quanto determinismos.`,
      term => `O tratamento de ${term} pressupõe uma leitura interdisciplinar.`,
      term => `A noção de ${term} opera como eixo da argumentação.`,
      term => `A discussão desloca ${term} para um horizonte comparativo.`,
      term => `O debate sobre ${term} expõe uma ambivalência estrutural.`,
      term => `A interpretação de ${term} depende do regime discursivo adotado.`,
      term => `A formulação de ${term} condensa impasses filosóficos e sociais.`,
    ],
  };

  const pluralTemplates: Array<(term: string) => string> = [
    term => `Há ${term} importantes neste contexto.`,
    term => `Revisei ${term} antes de terminar a atividade.`,
    term => `Comparei ${term} para entender melhor a situação.`,
    term => `Anotei ${term} que ainda preciso praticar.`,
    term => `Encontrei ${term} semelhantes em outro exercício.`,
    term => `Usei ${term} em uma frase curta.`,
    term => `Expliquei ${term} com exemplos simples.`,
    term => `Relacionei ${term} com a situação do texto.`,
    term => `Separei ${term} por tema para revisar melhor.`,
    term => `Ouvi ${term} em uma conversa natural.`,
    term => `Escrevi ${term} no meu caderno de revisão.`,
    term => `Marquei ${term} que preciso rever amanhã.`,
    term => `Pratiquei ${term} em voz alta.`,
    term => `Vi ${term} em contextos diferentes.`,
    term => `Transformei ${term} em exemplos próprios.`,
  ];

  if (isPlural) return pluralTemplates[seed % pluralTemplates.length](word);

  return templates[level][seed % templates[level].length](word);
}

const supplementalVocabularySeeds: Record<CEFRLevel, { bases: TermSeed[]; topics: TermSeed[]; example: (term: TermSeed, index: number) => string }> = {
  A1: {
    ...
    example: (term, index) => naturalSupplementalExample('A1', term, index),
  },
  A2: {
    ...
    example: (term, index) => naturalSupplementalExample('A2', term, index),
  },
  B1: {
    ...
    example: (term, index) => naturalSupplementalExample('B1', term, index),
  },
  B2: {
    ...
    example: (term, index) => naturalSupplementalExample('B2', term, index),
  },
  C1: {
    ...
    example: (term, index) => naturalSupplementalExample('C1', term, index),
  },
  C2: {
    ...
    example: (term, index) => naturalSupplementalExample('C2', term, index),
  },
};
...
function buildSupplementalVocabularyDecks(baseDecks: VocabularyDeck[]) {
  ...
  for (const level of ['A1','A2','B1','B2','C1','C2'] as CEFRLevel[]) {
    ...
    for (const term of candidateTerms) {
      ...
      words.push({ pt: term.pt, zh: term.zh, example: config.example(term, words.length) });
    ...
    if (words.length < neededDecks * 10) {
      for (const prefix of prefixes) {
        for (const term of directTerms) {
          ...
          existingWords.add(key);
          words.push({ pt: derived.pt, zh: derived.zh, example: config.example(derived, words.length) });
          if (words.length >= neededDecks * 10) break;
        }
        if (words.length >= neededDecks * 10) break;
      }
