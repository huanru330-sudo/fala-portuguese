...
const verbQuestions = [
  { infinitive: 'ter', subject: 'eu', sentence: 'Quando eu ___ tempo, vou estudar português.', accepted: [{ form: 'tiver', zh: 'quando 引出尚未发生的未来条件，所以用将来虚拟式 tiver。', pt: 'Quando introduz uma condição futura ainda não realizada; por isso usamos tiver.' }] },
  { infinitive: 'falar', subject: 'eu', sentence: 'Eu ___ português todos os dias.', accepted: [{ form: 'falo', zh: 'falo 表示现在每天都做的习惯。', pt: 'Falo indica um hábito atual.' }, { form: 'falava', zh: 'falava 也成立，表示过去每天说、现在未必继续。', pt: 'Falava também é possível: descreve um hábito no passado.' }] },
  { infinitive: 'poder', subject: 'nós', sentence: 'Nós ___ viajar amanhã.', accepted: [{ form: 'podemos', zh: 'podemos 表示按照当前情况，明天可以去。', pt: 'Podemos apresenta a possibilidade como válida agora.' }, { form: 'poderemos', zh: 'poderemos 也成立，更明确地把“能够”放在未来。', pt: 'Poderemos também é correto e coloca a possibilidade explicitamente no futuro.' }] },
  { infinitive: 'chegar', subject: 'vocês', sentence: 'Quando vocês ___, começaremos a reunião.', accepted: [{ form: 'chegarem', zh: '主句是将来，quando 后用将来虚拟式 chegarem。', pt: 'Com referência ao futuro, usamos chegarem depois de quando.' }] },
  { infinitive: 'estar', subject: 'ela', sentence: 'Se ela ___ aqui, ficaria feliz.', accepted: [{ form: 'estivesse', zh: 'se + 过去虚拟式搭配条件式 ficaria，表示与现实有距离的假设。', pt: 'Se + imperfeito do subjuntivo combina com ficaria para uma hipótese distante da realidade.' }] },
  { infinitive: 'chegar', subject: 'eles', sentence: 'Eles ___ cedo todos os dias.', accepted: [{ form: 'chegam', zh: 'chegam 表示现在的日常习惯。', pt: 'Chegam descreve um hábito atual.' }, { form: 'chegavam', zh: 'chegavam 也成立，表示过去反复发生的习惯。', pt: 'Chegavam também é correto para um hábito repetido no passado.' }] },
];
...
function PracticeHub({ c, language, onHome }: { c: Copy; language: Language; onHome: () => void }) {
  ...
  const journeyStages: Array<{ mode: PracticeMode; icon: string; title: string; meta: string }> = [
    ...
    { mode: 'verbs', icon: 'V', title: ui.verbs, meta: ui.verbsMeta },
  ];
      ...
      if (saved && typeof saved === 'object') {
        ...
        for (const level of cefrLevels) {
          ...
          const history = Object.fromEntries(Object.entries(current.history || {}).map(([date, record]) => [date, {
            ...record,
            completedStages: record.completed ? 5 : Math.min(record.completedStages, 4),
          }]));
          migrated[level] = { ...current, stage: Math.min(current.stage, 4), history };
        }
  ...
  if (mode === 'hub') return <div className="screen-enter min-h-[790px] px-6 pb-28 pt-7">
    ...
    <section className="journey-status mt-6"><div><span>{language==='zh'?'今日闯关进度':'Progresso de hoje'}</span><b>{todayLearningRecord?.completedStages || 0}/{journeyStages.length}</b></div><div><span style={{width:`${Math.max(5, ((todayLearningRecord?.completedStages || 0) / journeyStages.length) * 100)}%`}}/></div><small>{todayLearningRecord?.completed?(language==='zh'?'今天的任务已完成；五关均可点击复习，明天自动进入下一课。':'Tarefa concluída. Você pode rever qualquer fase; amanhã continuará na próxima lição.'):(language==='zh'?'完成一关自动解锁下一关；已完成关卡可以返回复习且不重复计入进度。':'Conclua uma fase para liberar a próxima. Fases concluídas podem ser revistas sem duplicar o progresso.')}</small></section>
    ...
    <Nav c={c} active="practice" onPractice={()=>{}}/>
  </div>;
  ...
  return <div className="screen-enter min-h-[790px] px-6 pb-8 pt-7">
    ...
    {mode === 'verbs' && <section className="study-panel mt-8 rounded-[28px] p-6">
      {!verbComplete ? <>
        <div className="verb-daily-status"><div><p className="eyebrow">{ui.verbTitle}</p><strong>{language==='zh'?`第 ${verbStep+1} 题 / 5`:`Questão ${verbStep+1} / 5`}</strong></div><span>{language==='zh'?`答对 ${verbScore}`:`Acertos ${verbScore}`}</span></div>
        ...
        <span className="completion-medal" aria-hidden="true">★</span><p className="eyebrow">{language==='zh'?'今日动词变位完成':'Conjugação concluída'}</p><h1>{verbScore}/5</h1><p>{language==='zh'?'你已完成5题，今日第5关已通过。':'Você concluiu 5 questões e terminou a fase de hoje.'}</p>
        ...
        <button className="next-question" onClick={returnToJourney}><span>{language==='zh'?'完成今日任务，返回地图':'Concluir e voltar ao mapa'}</span><b aria-hidden="true">→</b></button>
...
function Nav({ c, active, onPractice }: { c: Copy; active: 'practice'|'review'; onPractice: () => void }) { return <nav className="absolute-nav flex items-center justify-around" aria-label={c.navigation}><button onClick={onPractice} className={active==='practice'?'nav-active':''}>◉<span>{c.nav[1]}</span></button><button>◎<span>{c.nav[3]}</span></button></nav>; }
